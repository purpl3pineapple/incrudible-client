...
const { feedbackWorkflow } = require("./fixtures/feedback-workflow");

const withFeedback = { feedback: feedbackWorkflow };

test("renders the feedback schema from init options", async ({ page }) => {
  const errors = await openFixture(page, { init: withFeedback });
...
test("loads feedback records through the server envelope", async ({ page }) => {
  const errors = await openFixture(page, { init: withFeedback });
...
test("caps a region at two alerts, newest first", async ({ page }) => {
  const errors = await openFixture(page);
