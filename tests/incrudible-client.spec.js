...
test("handles date policies and image upload validation", async ({ page }) => {
	...
	const result = await page.evaluate(async () => {
		...
		const dates = {
			...
			business: DAYS.daysElapsed("2026-07-17", "2026-07-20", { businessDays: true }),
		...
		let nonImageError;
		try {
			await APP.imageToUpload(new File(["hello"], "proof.txt", { type: "text/plain" }));
		} catch (error) {
...
test("runs a rendered workflow with rules, validation, and preview", async ({ page }) => {
	const errors = await openFixture(page);
	...
	expect(await page.locator("#app-form").evaluate(form => form.checkValidity())).toBe(false);

	await page.locator("#review-date").fill("2026-07-27");
	await page.locator("#outcome").selectOption({ label: "Customer unavailable" });
	await expect(page.locator("#preview-list")).toContainText(
		"Customer unavailable on 2026-07-27 (reviewed 2026-07-27)",
	);
	expect(await page.locator("#app-form").evaluate(form => form.checkValidity())).toBe(true);

	await page.locator("#urgent").check();
	await expect(page.locator("#confirm-modal")).toHaveAttribute("open", "");
	await expect(page.locator("#confirm-modal-header")).toHaveText("Confirm escalation");
	await expect(page.locator("#urgent").locator("xpath=.." )).toContainText("Escalate immediately.");
	...
	await expect(page.locator("#reason")).toBeDisabled();
	await expect(page.locator("#contact")).toBeDisabled();
	const submittedNames = await page.locator("#app-form").evaluate(form =>
		[...new FormData(form)].map(([name]) => name),
	);
...
test("keeps dynamic lists correctly indexed and resets extra rows", async ({ page }) => {
	...
	const rows = await page.locator("#tags input").evaluateAll(inputs =>
		inputs.map(input => ({ id: input.id, name: input.name, value: input.value })),
	);
	expect(rows).toEqual([
...
test("synchronizes theme, loading, tabs, and current-day records", async ({ page }) => {
	const errors = await openFixture(page);

	await page.locator("#theme-toggle").check();
	await expect(page.locator("html")).toHaveAttribute("data-theme", "dark");
	expect(await page.evaluate(() => localStorage.getItem(APP.THEME_STORAGE_KEY))).toBe("dark");
...
test("publishes dialog outcomes and dismisses messages", async ({ page }) => {
	...
	await page.evaluate(() => {
		window.dialogEvents = [];
		APP.subscribe("confirm:accepted", detail => dialogEvents.push(["accepted", detail]));
		APP.subscribe("confirm:cancelled", detail => dialogEvents.push(["cancelled", detail]));
		APP.confirm("Proceed?", {
	...
	await expect(page.locator("#message-modal-dismiss-button")).toHaveText("Done");
...
test("handles Apps Script success and both failure paths", async ({ page }) => {
	...
	const installServer = outcome => page.evaluate(value => {
	...
	await installServer({ type: "success", response: { success: true, data: { id: 7 } } });
	const success = await page.evaluate(() => new Promise(resolve => {
		APP.runServer("save", ["value"], { onData: resolve });
	}));
	expect(success).toEqual({ id: 7 });
	expect(await page.evaluate(() => window.serverCall)).toEqual({ method: "save", args: ["value"] });
	await expect(page.locator("#app-overlay")).not.toHaveClass(/active/);
	...
	await expect(page.getByRole("status").last()).toHaveText("Couldn't save: Rejected");
	await expect(page.locator("#app-overlay")).not.toHaveClass(/active/);
	...
	await expect(page.getByRole("status").last()).toHaveText("Couldn't save: Offline");
	await expect(page.locator("#app-overlay")).not.toHaveClass(/active/);
	expectCleanPage(errors);
});

test("copies only valid preview content and reports clipboard failures", async ({ page }) => {
	...
	await page.evaluate(() => {
		APP.workflowLabel = "Review";
		APP.formControls.replaceChildren(APP.renderEntries([
			{
				type: "text",
				id: "case-number",
				name: "caseNumber",
	...
	await expect(page.getByRole("status").last()).toHaveText("Copied to clipboard.");

	await page.evaluate(() => {
		window.clipboardShouldFail = true;
	});
	await page.locator("#copy-preview").click();
	await expect(page.getByRole("status").last()).toHaveText("Couldn't copy to clipboard.");
	expectCleanPage(errors);
});